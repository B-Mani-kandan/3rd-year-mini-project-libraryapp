...
const Dasboard = ({ BooksData, UserData, requestedBooks, returnBooks }) => {
  ...
  let totalNumberOfBooksAvaliable = Books.filter(
    (d) => d.currentHolder === "libary"
  ).length;
  ...
  return (
    <div>
      ...
      <main style={{ maxWidth: "1100px", margin: "50px auto" }}>
        ...
        <h1 style={{ margin: "120px 0 30px 0", fontSize: "40px" }}>
          Books Mangement
        </h1>
        <AboutBooks
          setLoading={setLoading}
          setMyRequestedBooks={setMyRequestedBooks}
          bookList={booksList()}
          BooksData={Books}
        />
        ...
        <section>
          ...
          <div>
            {myRequestedBooks.length === 0 && (
              <h2>
                If you want get book from libary then Please Make Get Request{" "}
              </h2>
            )}
            {myRequestedBooks.length > 0 &&
              myRequestedBooks.map((d) => (
                <div
                  key={d._id}
                  style={{
        ...
        <section>
          ...
          <div>
            {rBooks.length === 0 && (
              <h2>
                If you want return your book to libary then Please Make Return
                Request{" "}
              </h2>
            )}
            {rBooks.length > 0 &&
...
export async function getServerSideProps(ctx) {
  ...
  const Books = await fetch(`${URL}/api/hello`, {
    headers: {
      method: "GET",
      "Content-Type": "application/json",
      Authorization: token,
